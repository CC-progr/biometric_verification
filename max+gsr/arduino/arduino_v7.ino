...
int get_data_callback(size_t offset, size_t length, const void *context)
{
    // Realiza una conversión explícita a int antes de devolver el valor
    Serial.print("*GET_DATA_CALLBACK* RETURNS: ");
    Serial.println(sensorValue);
    //Serial.println(*(reinterpret_cast<const int *>(context)));
    return sensorValue;
}

void loop()
{
  ei_printf("Edge Impulse is working\n");
  ...
  while (1)
  {
    ...
    // double sensorValuedouble = static_cast<double>(sensorValue);
    //signal.get_data_context = &sensorValueFloat;
    signal.get_data = &get_data_callback;
    // signal.get_data = sensorValue;

    //Serial.println("3");

    ei_impulse_result_t result = { 0 };
    EI_IMPULSE_ERROR r = run_classifier(&signal, &result, debug_nn); // Pasamos la dirección de sensorValue como contexto
